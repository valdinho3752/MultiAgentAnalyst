...
COLLECTION_NAME = "rag_metadata_demo_finetunned"
# Apuntar al host desde Docker para ver el Qdrant real
QDRANT_URL = os.getenv("QDRANT_URL", "http://host.docker.internal:6333")

# Definir path relativo al modelo
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_DIR = os.path.join(BASE_DIR, "..", "metadata", "EntidadesFinancieras", "modelos", "bolivia-finance-onnx")
MODEL_PATH = os.path.normpath(MODEL_DIR)

# --- CLASE LOCAL TOTALMENTE COMPATIBLE ---
class LocalEmbedding(OnnxTextEmbedding):
    def __init__(self, model_path):
        self._model_dir = model_path
        self.model_name = os.path.basename(model_path)
        
        # ATRIBUTOS OBLIGATORIOS (Plural y Singular para evitar errores)
        self.cache_dir = model_path
        self._local_files_only = True
        self._specific_model_path = model_path
        self.parallel = None
    ...
    def add_extra_session_options(self, session_options):
        pass
    ...
    def _load_model(self):
        ...
        self.model = ort.InferenceSession(onnx_path, providers=self.providers)
...
embedding_model = LocalEmbedding(MODEL_PATH)
